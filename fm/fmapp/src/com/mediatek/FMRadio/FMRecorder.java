...
//import android.os.FileUtils;//xbw
...
public class FMRecorder implements MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener,
    ...
    public void startRecording(Context mcontext) {
      ...
      // mSDCardID = FileUtils.getFatVolumeId(sdPath); //xbw
      ...
      mSDCardID = actualVolume.getFatVolumeId();
      /*
        ...
        for (int i = 0; i < volumeList.length; i++) {
            ...
            if (path.startsWith(sdPath)) {
                ...
                if (path.length() == length || path.charAt(length) == '/') {
                    mSDCardID = volumeList[i].getStorageId();
                    break;
                }else{
		   mSDCardID = actualVolume.getFatVolumeId();
		}
     ...
     */
    public void onSDInserted(Context mcontext) {
        LogUtils.d(TAG, ">> onSDInserted");
        //int newCardID = FileUtils.getFatVolumeId(Environment.getExternalStorageDirectory().getPath()); //xbw
       StorageManager storageManager =
                (StorageManager)mcontext.getSystemService(Context.STORAGE_SERVICE);
        final StorageVolume actualVolume = storageManager.getPrimaryVolume();
        int newCardID = actualVolume.getFatVolumeId();
     /*  
     int  mSDCardID;
      StorageVolume[] volumeList = storageManager.getVolumeList();
        for (int i = 0; i < volumeList.length; i++) {
            String volumePath = volumeList[i].getPath();
            if (storageManager.path.startsWith(sdPath)) {
                int length = storageManager.volumePath.length();
                if (storageManager.path.length() == length || storageManager.path.charAt(length) == '/') {
                    mSDCardID = volumeList[i].getStorageId();
                    break;
                }else{
		   mSDCardID = actualVolume.getFatVolumeId();
